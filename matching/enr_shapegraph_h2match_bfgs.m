% enr_shapegraph_h2match_bfgs.m: function that evaluates:
% a) the discretized matching functional for shape graph registration using second order elastic Sobolev
%    metrics (H2 metrics) - to be used when using the smooth approximation of the TV norm regularizer.
...
function [ENR, dENR] = enr_shapegraph_h2match_bfgs(x)


% Note: template = source (c0), and data = target (c1)
global template data objfunc optimc gradavg
...
switch lower(objfun.edge_weight)
    ...
    case 'both'  % if optimizing over both transformed source and target edge weights
        ...
        % Compute TV norm of transformed source edge weights rho0, and associated gradient
        [tvSource, dTvSource_sourceRho] = tv_norm(transfSource);
        
        % Compute TV norm of target edge weights rho1, and associated gradient
        [tvTarget, dTvTarget_targetRho] = tv_norm(transfTarget);
...
switch lower(objfun.edge_weight)
    ...
    case 'both'  % if optimizing over both transformed source and target edge weights
        ...
        ENR = energyPath + lambdaVar*varDist + ...
                 lambdaTvSource*tvSource + lambdaTvTarget*tvTarget + ...
        ...
        dENR = [ u*dENRh2_curvePath(:); ...  % wrt path of component curves of shape graph
                 lambdaVar*dVarDist_sourceRho + lambdaTvSource*dTvSource_sourceRho + ...
                     lambdaPenSource*dPenSource_sourceRho; ...  % wrt to transformed source edge weights
                 lambdaVar*dVarDist_targetRho + lambdaTvTarget*dTvTarget_targetRho + ...
    ...
    case 'source'  % if optimizing over transformed source edge weights only
        ...
        % Evaluate matching functional
        ENR = energyPath + lambdaVar*varDist + ...
                    lambdaTvSource*tvSource + lambdaPenSource*sum(penSource);
    ...
    case 'target'  % if optimizing over target edge weights only
        ...
        % Evaluate matching functional
        ENR = energyPath + lambdaVar*varDist + ...
                      lambdaTvTarget*tvTarget + lambdaPenTarget*sum(penTarget);
