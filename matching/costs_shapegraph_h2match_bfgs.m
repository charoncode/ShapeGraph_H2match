% costs_shapegraph_h2match_bfgs.m: function that evaluates the costs in the discretized matching functional for shape graph
% registration using second order elastic Sobolev metrics (H2 metrics) - when using the smooth approximation of 
...
%       c) tv_source    : tv norm of transformed source edge weights rho0
%       d) tv_target    : tv norm of target edge weights rho1
...
function [ENR, optPath, transfSource, transfTarget] = costs_shapegraph_h2match_bfgs(x)
...
switch lower(objfun.edge_weight)
    ...
    case 'both'  % if optimizing over both transformed source and target edge weights
        ...
        % Compute TV norm of transformed source edge weights rho0
        [ENR.tv_source, ~] = tv_norm(transfSource);
        
        % Compute TV norm of target edge weights rho1
        [ENR.tv_target, ~] = tv_norm(transfTarget);
        ...
        ENR.energy_min = ENR.energy_path + lambdaVar*ENR.varifold_cost + ...
                 lambdaTvSource*ENR.tv_source + lambdaTvTarget*ENR.tv_target + ...
    ...
    case 'source'  % if optimizing over transformed source edge weights only
        ...
        % Evaluate matching functional
        ENR.energy_min = ENR.energy_path + lambdaVar*ENR.varifold_cost + ...
                 lambdaTvSource*ENR.tv_source + ...
                    lambdaPenSource*ENR.pen_source;
    ...
    case 'target'  % if optimizing over target edge weights only
        ...
        % Evaluate matching functional
        ENR.energy_min = ENR.energy_path + lambdaVar*ENR.varifold_cost + ...
                 lambdaTvTarget*ENR.tv_target + ...
                    lambdaPenTarget*ENR.pen_target;
